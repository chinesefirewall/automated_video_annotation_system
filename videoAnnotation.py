...
iClass = []
key = -1
color = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 0, 255), (0, 255, 255), (0, 51, 102), (102, 51, 0),
         (0, 0, 0), (255, 255, 255)]
...
while cap.isOpened() and ret:
    ...
    if key == 32:
        ...
        save = False
        for values in range(0, len(startRect)):
            frameWidth, frameHeight = abs(startRect[values][0] - endRect[values][0]), abs(
                startRect[values][1] - endRect[values][1])
            xCenter, yCenter = abs((startRect[values][0] + endRect[values][0]) / 2.0), abs(
                (startRect[values][1] + endRect[values][1]) / 2.0)
...
command = 'ls -d ' + os.getcwd() + '/{}/JPEGImages/* > '.format(foldName) + os.getcwd() + '/{}/imgList.txt'.format(
    foldName)
os.system(command)
